    ...
    public function update(Request $request)
{
    ...
    $request->validate([
        ...
        'family_situation' => 'nullable|string',
        'gender' => 'nullable|string',
        'picture' => 'nullable|file|mimes:jpeg,png,jpg,gif,svg|max:2048',
    ]);
    ...
    // Gérer la mise à jour ou l'ajout de la photo de profil
    if ($request->hasFile('picture')) {
        // Vérifier si une ancienne photo existe, puis la supprimer
        if ($user->picture) {
            // Utiliser le disque 'public' pour supprimer l'image précédente
            Storage::disk('public')->delete('profil_picture/' . $user->picture);
        }

        // Stocker la nouvelle image et récupérer son chemin
        $image_name = $request->file('picture')->store('profil_picture', 'public');

        // Mettre à jour le champ 'picture' avec le nom de l'image
        $user_data['picture'] = basename($image_name);  // Récupérer uniquement le nom du fichier
    }
    ...
    return response()->json([
        ...
        'user' => $user,
        'picture_url' => asset('storage/profil_picture/' . $user->picture),
    ], 200);
}
